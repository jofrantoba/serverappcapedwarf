...
public final class Iterators {
  ...
  /**
   * Iterates through two iterators in parallel, returning pairs of items. The
   * iterator will return {@code false} for {@code hasNext()} when the end of
   * <i>either</i> iterator is reached. The returned iterator does not support
   * {@code remove()}.
   *
   * @return an iterator that returns pairs of items from the {@code first} and
   *         {@code second} iterators
   */
  @GoogleInternal // see Pair
  public static <A, B> Iterator<Pair<A, B>> pairUp(
      final Iterator<A> first, final Iterator<B> second) {
    checkNotNull(first);
    checkNotNull(second);
    return new Iterator<Pair<A, B>>() {
      ...
      /*
       * See reviewlog discussion of CL 2984305 for ideas and concerns about
       * implementing remove in the future.
       */
      @Override
      public void remove() {
