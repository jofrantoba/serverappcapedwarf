...
public abstract class FluentIterable<E> implements Iterable<E> {
  ...
  @Beta
  @Deprecated
  /* MOE:insert public */ static <E> FluentIterable<E> of(E[] elements) {
   ...
   *
   * <p><b>{@code Stream} equivalent:</b> {@code stream.reduce((a, b) -> { throw new
   * IllegalArgumentException(); }).get()} (but note that a different exception type such as
   * {@link VerifyException} may be more appropriate).
   *
  ...
  public final E getOnlyElement() {
    return Iterables.getOnlyElement(getDelegate());
  }

  /**
   * Returns the single element contained in this fluent iterable, or {@code defaultValue} if this
   * fluent iterable is empty.
   *
   * <p><b>{@code Stream} equivalent:</b> {@code stream.reduce((a, b) -> { throw new
   * IllegalArgumentException(); }).orElse(defaultValue)} (but note that a different exception type
   * such as {@link VerifyException} may be more appropriate).
   *
  ...
  public final E getOnlyElement(@Nullable E defaultValue) {
    return Iterables.getOnlyElement(getDelegate(), defaultValue);
  }
  ...
  /**
   * Divides this fluent iterable into unmodifiable fluent iterables of the given size (the final
   * iterable may be smaller). For example, partitioning a fluent iterable containing
   * {@code [a, b, c, d, e]} with a partition size of 3 yields {@code [[a, b, c], [d, e]]} -- an
   * outer fluent iterable containing two inner fluent iterables of three and two elements, all in
   * the original order.
   *
   * <p>Iterators returned by the returned outer and innter fluent iterables do not support the
   * {@link Iterator#remove()} method.
   *
   * <p><b>{@code Stream} equivalent:</b> none.
   *
   * @param size the desired size of each partition (the last may be smaller)
   * @return a fluent iterable of fluent iterables containing the elements of this fluent iterable
  ...
  public final FluentIterable<FluentIterable<E>> partition(int size) {
    return from(Iterables.partition(getDelegate(), size)).transform(new FromIterableFunction<E>());
  }

  /**
   * Divides this fluent iterable into unmodifiable fluent iterables of the given size, padding the
   * final fluent iterable with null values if necessary. For example, partitioning a fluent
   * iterable containing {@code [a, b, c, d, e]} with a partition size of 3 yields
   * {@code [[a, b, c], [d, e, null]]} -- an outer fluent iterable containing two inner fluent
   * iterables of three elements each, all in the original order.
   *
   * <p>Iterators returned by the returned fluent iterables do not support the
   * {@link Iterator#remove()} method.
   *
   * <p><b>{@code Stream} equivalent:</b> none.
   *
   ...
   * @return a fluent iterable of fluent iterables containing the elements of this fluent iterable
   *     divided into partitions (the final fluent iterable may have trailing null elements)
   * @throws IllegalArgumentException if {@code size} is nonpositive
   */
  // TODO(kevinb): return FluentIterable<List<E>> instead
  @GoogleInternal
  public final FluentIterable<FluentIterable<E>> partitionWithPadding(int size) {
    return from(Iterables.paddedPartition(getDelegate(), size))
