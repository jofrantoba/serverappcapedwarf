...
public final class Iterables {
  ...
  /**
   * Iterates through two iterables in parallel, returning pairs of items. The returned iterable's
   * iterator will return {@code false} for {@code hasNext()} when the end of <i>either</i> iterable
   * is reached. The iterator does not support {@code remove()}.
   *
   * <p><b>Java 8 users:</b> The {@code Stream} equivalent of this method is {@code
   * Streams.zip(firstStream, secondStream, Pair::of)} (often you can skip creating pair instances
   * and just do what you're trying to do directly).
   *
   * @return an iterable whose iterator returns pairs of items from the {@code first} and {@code
   *     second} iterables.
   */
  @GoogleInternal // see Pair
  public static <A, B> Iterable<Pair<A, B>> pairUp(
      final Iterable<A> first, final Iterable<B> second) {
    checkNotNull(first);
    checkNotNull(second);
    return new FluentIterable<Pair<A, B>>() {
      @Override
      public Iterator<Pair<A, B>> iterator() {
        return Iterators.pairUp(first.iterator(), second.iterator());
      }
    };
  }
  ...
  // TODO(jord): Is this the best place for this? Move to fluent functions?
